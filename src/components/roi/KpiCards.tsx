...
export default function KpiCards({ netProfit, operatingProfit, breakEven, currentVolume }: Props) {
  ...
  return (
    <View style={{
      ...
      <Text style={{
        fontSize: FS.caption, fontWeight: 600, color: '#9298a8',
        display: 'block', marginBottom: '6px', marginTop: '4px',
      }}>
        {t('roi.kpi_row_safety')}
      </Text>

      {/* Row 2: 盈亏平衡收入 + 营业利润 + 安全边际率 */}
...
function KpiCard({
  ...
  return (
    ...
    >
      <Text style={{ fontSize: FS.heading, display: 'block' }}>{icon}</Text>
      <Text style={{
        fontSize: FS.kpi, fontWeight: 800, color: valueColor,
